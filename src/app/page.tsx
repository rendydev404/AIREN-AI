...
export default function Home() {
  ...
  const [lastSentImageBase64, setLastSentImageBase64] = useState<string | null>(
    null
  );
  const [lastSentImageMimeType, setLastSentImageMimeType] = useState<
    string | null
  >(null);
  ...
  const creatorKeywords = [
    ...
    "who is your boss",
    "pemilik kamu siapa",
    "rendi irawan itu siapa",
    "apakah kamu dari google",
    "apa kamu produk google",
    "are you a google product",
    "is google your maker",
    "kamu google punya bukan",
    "apakah rendi irawan itu google",
    "kamu ai google ya",
    "kamu ai punya google",
    "kamu dibuat google",
    "are you made by google",
    "your manufacturer",
    "siapa yang merancangmu",
    "siapa arsitekmu",
    "siapa yang ngoding kamu",
    "siapa programmer kamu",
    "siapa yang program kamu",
    "who programmed you",
    "your programmer",
    "apakah kamu ciptaan google",
    "kamu bukan buatan google kan",
    "kamu google punya?",
  ];

  useEffect(() => {
    // Splash screen SELALU tampil 3 detik setiap reload
    showSplashScreen();
  }, []);
  ...
  const showSplashScreen = () => {
    if (splashScreenRef.current) {
      try {
        const splashBg = splashScreenRef.current.querySelector("#splash-bg");
        const splashLogo =
          splashScreenRef.current.querySelector("#splash-logo");
        if (splashBg && splashLogo) {
          gsap.set(splashBg, { x: 0 });
          gsap.set(splashLogo, { opacity: 0 });
          gsap.to(splashBg, { x: 100, duration: 1.5, ease: "power1.inOut" });
          gsap.to(splashLogo, {
            opacity: 1,
            duration: 1.5,
            ease: "power1.inOut",
  ...
  const initApp = () => {
    ...
    setTimeout(() => {
      ...
      if (!hasSeenWelcome) {
        ...
        addMessage(
          "Salam kenal! Saya AIREN-AI, sebuah inovasi AI yang sepenuhnya dikonsep dan dibangun oleh <strong>AIREN-TEAM</strong>. Saya siap membantu Anda dengan kemampuan baru saya! Ada yang bisa saya proses hari ini?",
          "ai"
        );
      }
    }, 200);
  ...
  const processAndSendToAI = async (
    ...
    if (creatorKeywords.some((keyword) => lowerCaseMessage.includes(keyword))) {
      ...
      const randomResponse =
        responsesRendi[Math.floor(Math.random() * responsesRendi.length)];
    ...
    try {
      ...
      if (currentConversationParts.length > 0) {
        setChatHistory((prev) => [
          ...prev,
          { role: "user", parts: currentConversationParts },
        ]);
      }
      setChatHistory((prev) => [
        ...prev,
      ...
      if (imageBase64 && lastSentImageBase64) {
        setTimeout(() => {
          addMessage(
            ...
            {
              type: "imagineImage",
              imageBase64: lastSentImageBase64,
              imageMimeType: lastSentImageMimeType,
            }
          );
        }, 700);
      }
      ...
      updateLastMessage({
        message:
          "Aduh, sepertinya ada masalah dengan koneksi internetku. Pastikan kamu online dan coba lagi ya.",
        isLoading: false,
      });
      showInfoModal(
        "Error Koneksi",
  ...
  const handleImagineImage = async (actionData: any) => {
    if (!actionData.imageBase64 || !actionData.imageMimeType) {
      showInfoModal(
        "Info",
        "Tidak ada gambar yang terdeteksi untuk diimajinasikan."
      );
      return;
    }
    ...
    try {
      const imaginativeText = await GeminiAPI.imagineImage(
        actionData.imageBase64,
        actionData.imageMimeType
      );
      updateLastMessage({
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "model",
          parts: [{ text: `Imajinasi AIREN-AI: ${imaginativeText}` }],
        },
      ]);
    } catch (error) {
  ...
  const handleSummarize = async () => {
    ...
    let conversationText =
      "Berikut adalah transkrip percakapan antara Pengguna dan AIREN-AI (sebuah AI yang dibuat oleh Rendi Irawan):\n\n";
    ...
    gsap.fromTo(
      ...
      },
      {
        ...
        onComplete: () => {
          setTheme(newTheme);
          setTimeout(() => {
            gsap.to(themeTransitionRef.current, {
              opacity: 0,
              duration: 0.3,
              onComplete: () => {
                if (themeTransitionRef.current)
                  themeTransitionRef.current.style.display = "none";
              },
            });
          }, 200);
        },
  ...
  // Tambahkan efek animasi pada logo splash
  useEffect(() => {
    if (!splashShown && splashScreenRef.current) {
      const logoImg = splashScreenRef.current.querySelector(".splash-logo-img");
      if (logoImg) {
        gsap.fromTo(
          logoImg,
          { scale: 0.2, opacity: 0 },
          { scale: 1, opacity: 1, duration: 2, ease: "elastic.out(1, 0.4)" }
        );
      }
    }
  }, [splashShown]);
  ...
  if (!splashShown) {
    return (
      <div
        id="splash-screen"
        ref={splashScreenRef}
        className="fixed inset-0 flex items-center justify-center z-[9999] bg-[var(--bg-primary)] overflow-hidden"
      >
        <div
          id="splash-bg"
          ...
          style={{ zIndex: 1 }}
        />
        <div id="splash-logo" style={{ zIndex: 2 }}>
          <LogoImage className="w-60 h-60 md:w-80 md:h-80 lg:w-[28rem] lg:h-[28rem] splash-logo-img" />
        </div>
      </div>
    );
  }
          ...
          <div className="p-4 border-b border-[var(--border-color)]">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <LogoImage id="sidebar-logo" className="w-20 h-20" />
              </div>
              <button
                onClick={() => setIsSidebarOpen(false)}
      ...
      >
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2 lg:hidden">
            ...
            {/* PERUBAHAN: Tombol hamburger dipindahkan ke sini, di sebelah kanan ThemeToggle */}
            <button
          ...
          {/* Desktop kanan header (tidak ada perubahan di sini) */}
      ...
      >
        <div
          dangerouslySetInnerHTML={{
            __html: infoModalMessage.replace(/\n/g, "<br>"),
          }}
        />
      </Modal>
